...
const tbkgmbr = () => new Promise((resolve, reject) => {
    ...
    fetchJson(`https://api.vhtear.com/tebakgambar&apikey=${config.vhtear}`)
...
const simi = (chat) => new Promise((resolve, reject) => {
    ...
    fetchJson(`https://simsumi.herokuapp.com/api?text=${chat}&lang=id`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
...
const asupan = () => new Promise((resolve, reject) => {
    ...
    fetchText('https://auto.hzrossh.com/asupan/asupan.txt')
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
