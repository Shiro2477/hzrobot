 ...
 * @returns {Promise<object>}
 */
const jurnalotaku = (title) => new Promise((resolve, reject) => {
    console.log(`Get jurnalotaku search for ${title}...`)
    fetchJson(`https://api.vhtear.com/otakusearch?query=${title}&apikey=${config.vhtear}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
