...
/**
 * Get Search Anime Otakudesu.
 * @param {string} query 
 * @returns {Promise<object>} 
 */
const otaku = (query) => new Promise((resolve, reject) => {
    console.log(`Searching for ${query}...`)
    fetchJson(`https://lolhuman.herokuapp.com/api/otakudesusearch?apikey=${config.lol}&query=${query}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
...
const joox = (query) => new Promise((resolve, reject) => {
    console.log(`Searching for ${query} in joox...`)
    fetchJson(`https://api.vhtear.com/music?query=${query}&apikey=${config.vhtear}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
