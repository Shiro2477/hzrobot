...
const _antivirtext = JSON.parse(fs.readFileSync('./database/group/antivirtext.json'))
...
module.exports = msgHandler = async (bocchi = new Client(), message) => {
    try {
        ...
        const isAntiVirtextOn = isGroupMsg ? _antivirtext.includes(groupId) : false
        ...
        if (chats == 'bot') {
            if (!isGroupMsg) await bocchi.reply(from, `*Bot Aktif*, Untuk Memulai bot silahkan ketik ${prefix}menu`, id)
        }
        ...
        switch (command) {
            case 'antiporn': // Premium, chat VideFikri
                await bocchi.reply(from, 'Premium Feature!\n\nContact: wa.me/6285156022091?text=Buy%20Anti%20Porn', id)
            break
            ...
            case 'fb':
                ...
                downloader.fb(url)
                .then(async ({ result })=> {
                    const responses = await fetch(result.VideoUrl);
                    const buffer = await responses.buffer();
                    fs.writeFileSync(`./temp/${sender.id}_facebook.mp4`, buffer)
                    await bocchi.sendFile(from, `./temp/${sender.id}_facebook.mp4`, `${sender.id}_facebook.mp4`, '', id)
                    console.log('Success sending facebook video!')
                    fs.unlinkSync(`./temp/${sender.id}_facebook.mp4`)
                })
                .catch(async (err) => {
                    console.log(err)
                    await bocchi.reply(from, 'Error!', id)
            ...
            case 'ytmp3':
                    ...
                    .then(async ({result}) => {
                        ...
                        if (Number(size.split(' MB')[0]) >= 20.00) {
                            await bocchi.sendFileFromUrl(from, thumbnail, 'thumbnail.jpg', `➸ *Judul*: ${judul}\n➸ *ID*: ${id}\n➸ *Size*: ${size}\n\nGagal, maksimal size adalah *20MB*!\nSilahkan download sendiri melalui URL dibawah:\n${url}`, id)
                        } else {
                        await bocchi.sendFileFromUrl(from, thumbnail, 'thumbnail.jpg', `➸ *Judul*: ${judul}\n➸ *Size*: ${size}\n➸ *ID*: ${id}\n➸ *Extensiom*: ${extension}\n➸ *Source*: ${source}\n\nAudio Sedang dikirim, sabar ya...`, id)
                        await bocchi.sendFileFromUrl(from, url, 'ytmp3.mp3', '', id)
                        }
                    }) 
                    .catch(async (err) => {
                        console.error(err)
                ...
                downloader.ytdl(url)
                    .then(async (res) => {
                        ...
                        //} else if (Number(res.size(' MB')[0]) >= 50) {
            ...
            case 'ytdl4':
                ...
                .then(async ({result}) => {
                    ...
                    await bocchi.sendFileFromUrl(from, imgUrl, 'thumbnail.jpg', `➸ *Judul*: ${judul}\n➸ *ID*: ${id}\n➸ *Source*: ${source}\n\nVideo Sedang dikirim, sabar ya...`, id)
                    await bocchi.sendFileFromUrl(from, urlVideo, 'ytmp3.mp3', '', id)
                }) 
                .catch(async (err) => {
                    console.error(err)
                    await bocchi.reply(from, 'Error!', id)
            ...
            case 'tokhastag':
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                    downloader.tiktokht(q)
                    .then(async ({ result }) => {
                        let ttkht = `*「 TIKTOK HASTAG 」*\n `
                        for (let i = 0; i < 5; i++) {
                            const { name, nickName, title, urlVideo, source } = await result[i]
                            ttkht +=  `\n\n➸ *Title*: ${title}\n➸ *Name* : ${name}\n➸ *NickName*: ${nickName}\n➸ *Source*: ${source}\n➸ *URL*: ${urlVideo}\n\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                        }
                        ttkht += '\n\nBy: @hendy29_'
                        await bocchi.reply(from, ttkht, id)
                        console.log('Success sending tiktok hastag result!')
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'cocodl':
                ...
                if (!isPremium) return await bocchi.reply(from, ind.notPremium(), id)
                await bocchi.reply(from, ind.wait(), id)
                downloader.cocodl(url)
                    .then(async ({ result })=> {
                        const responses = await fetch(result.video);
                        const buffer = await responses.buffer();
                        fs.writeFileSync(`./temp/${sender.id}_cocofun.mp4`, buffer)
                        await bocchi.sendFile(from, `./temp/${sender.id}_cocofun.mp4`, `${sender.id}_cocofun.mp4`, '', id)
                        console.log('Success sending cocofun video!')
                        fs.unlinkSync(`./temp/${sender.id}_cocofun.mp4`)
                    })
                    .catch(async (err) => {
                        console.log(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'brainly':
                ...
                misc.brainly(q)
                    .then(async ({ result }) => {
                        ...
                        console.log('Success sending Brainly!')
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'wiki':
                ...
                misc.wiki(q)
                    .then(async ({ result }) => {
                        await bocchi.reply(from, result.Info, id)
                        console.log(`Success Search ${q} in Wikipedia`)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
    ...
    _keybot_ : ${prefix}jadwaltv tvri
        ...
        _*List TV Index*_`, id)
                ...
                misc.its(q)
                    .then(async ({ result }) => {
                        for (let i = 0; i < result.story.itemlist.length; i++) {
                            ...
                            await bocchi.sendFileFromUrl(from, urlDownload, '', 'By: hendy29_', id)
            ...
            case 'igstalk':
                ...
                misc.igStalk(q)
                    .then(async ({ result }) => {
                            ...
                            const { biography, follow, follower, full_name, is_private, picture, username,post_count } = result
                            const text = `*「 IG STALK 」*\n\n➸ *Username*: ${username}\n➸ *Full name*: ${full_name}\n➸ *Bio*: ${biography}\n➸ *Followers*: ${follower}\n➸ *Followings*: ${follow}\n➸ *Private*: ${is_private ? 'Yes' : 'No'}\n➸ *Total posts*: ${post_count}`
                            await bocchi.sendFileFromUrl(from, picture, 'insta.jpg', text, id)
            ...
            case 'tktstalk':
                ...
                .then(async ({ status, result }) => {
                    ...
                    let tktstlk = '-----[ *Tiktok STALK * ]-----'
                    for (let i = 0; i < result; i++) {
                        const { user_picture, username, nickname, bio, followers, followings, like, video} = result[i]
                        tktstlk += `\n\n➸ *Username*: ${username}\n➸ *Nick Name*: ${nickname}\n➸ *Bio*: ${bio}\n➸ *Followers*: ${followers}\n➸ *Followings*: ${followings}\n➸ *Like*: ${like}\n➸ *Jumlah Video*: ${video}`
                    }
                    await bocchi.reply(from, tktstlk, id)
            ...
            break
            case 'jurnalotaku': //by hendy29_
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                weeaboo.jurnalotaku(q)
                    .then(async ({ result }) => {
                        let jurnalotaku = `Respon Code: *${result.response}*`
                        for (let i = 0; i < result.data.length; i++) {
                            jurnalotaku +=  `\n\n➸ *Title:* : ${result.data[i].title}\n *Link* : ${result.data[i].link}\n➸ *Date Time*: ${result.data[i].datetime}\n\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                        }
                        jurnalotaku += '\n\nBy: @hendy29_'
                        await bocchi.reply(from, jurnalotaku, id)
                        console.log('Success sending jurnalotaku result!')
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'xxx':
                ...
                misc.xxx(q)
                    .then(async ({ result }) => {
                        let xxx = `*[[XXX SEARCH XTUBE]]*`
                        for (let i = 0; i < result.data.length; i++) {
                            xxx +=  `\n\n➸ *Title:* : ${result.data[i].title}\n *Duration* : ${result.data[i].duration}\n➸ *URL*: ${result.data[i].url}\n\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                        }
                        xxx += '\n\nBy: @hendy29_'
                        await bocchi.reply(from, xxx, id)
                        console.log('Success sending xxx result!')
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'xtube':
                ...
                misc.xtube(url)
                    .then(async ({ result }) => {
                        let xtube = `Result for: *${result.title}*`
                        for (let i = 0; i < result.urlVideo.length; i++) {
                             if (result.urlVideo[i].format === 'mp4') {
                                await bocchi.sendFileFromUrl(from, result.urlVideo[i].videoUrl, 'xtube.mp4', `*...:* *Xtube Downloader* *:...*\n\nTitle: ${result.title}\n *Quality:* : ${result.urlVideo[i].quality}\n`, id)
                            }
                        }
                        console.log('Success sending xtube videos!')
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'yts':
                ...
                await bocchi.reply(from, ind.wait(), id)
                    ...
                    .then(async ({ result }) => {
                        let txt = `*「 YT SEARCH 」*\n`
                        for (let i = 0; i < 5; i++) {
                            const { urlyt, title, channel, duration, views } = await result[i]
                            txt +=  `\n\n➸ *Judul*: ${title}\n➸ *Channel*: ${channel}\n➸ *Durasi*: ${duration}\n➸ *Views*: ${views}\n➸ *Link*: ${urlyt}\n\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                        }
                        txt += '\n\nBy: @hendy29_'
                        await bocchi.reply(from, txt, id)
        ...
        break
            ...
            case 'music':
                ...
                misc.music(q)
                    .then(async ({ result }) => {
                        if (Number(result.size.split(' MB')[0]) >= 10.0) return bocchi.sendFileFromUrl(from, result.thumbnail, `${result.title}.jpg`, `Judul: ${result.title}\nSize: *${result.size}*\n\nGagal, Maksimal video size adalah *10MB*!`, id)
                        await bocchi.sendFileFromUrl(from, result.thumbnail, `${result.title}.jpg`, ind.music(result), id)
            ...
            case 'trentwt':
                ...
                .then(async ({ result }) => {
                    ...
                    for (let i = 0; i < result.data.length; i++) {
                        txt +=  `\n\n➸ *Hashtag* : ${result.data[i].title}\n➸ *Tweets*: ${result.data[i].tweet_count}\n➸ *Link*: ${result.data[i].url}\n\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                    }
                    txt += '\n\nBy: @hendy29_'
                    await bocchi.reply(from, txt, id)
                    console.log('Success sending TRANDING TWITTER result!')
                })
                .catch(async (err) => {
                    console.error(err)
                    await bocchi.reply(from, 'Error!', id)
            ...
            case 'bola':
                ...
                misc.bola()
                .then(async ({ result }) => {
                    let txt = `*「 JADWAL BOLA LIVE 」*`
                    for (let i = 0; i < result.data.length; i++) {
                        txt +=  `\n\n➸ *Kickoff* : ${result.data[i].kickoff}\n➸ *Pertandingan*: ${result.data[i].pertandingan}\n➸ *TV*: ${result.data[i].stasiuntv}\n\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                    }
                    txt += '\n\nBy: @hendy29_'
                    await bocchi.reply(from, txt, id)
                    console.log('Success sending JADWAN BOLA result!')
                })
                .catch(async (err) => {
                    console.error(err)
                    await bocchi.reply(from, 'Error!', id)
            ...
            case 'spamsms':
                ...
                await bocchi.reply(from, ind.wait(), id)
                misc.spamsms(q)
                    .then(async ({ status, result, message }) => {
                        if (status !== 200) {
                            await bocchi.reply(from, message, result, id)
                        } else {
            ...
            case 'help':
                ...
                if (args[0] === '1') {
                    ...
                    if (!isPremium) return await bocchi.reply(from, ind.notPremium(), id)
                    await bocchi.sendText(from, ind.menuPrem())
                } else {
            ...
            case 'tos':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                await bocchi.sendLinkWithAutoPreview(from, 'https://github.com/Shiro2116', ind.tos(ownerNumber))
            break
            case 'info':
                const juUser = _registered.length
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                await bocchi.reply(from, ind.info())
            break
            case 'join':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
            ...
            case 'limit':
                ...
                await bocchi.reply(from, `⤞ Limit left: ${limit.getLimit(sender.id, _limit, limitCount)} / 30\n\n*_Limit direset pada pukul 00:00 WIB_*`, id)
            break
            ...
            case 'bts':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await bocchi.reply(from, ind.wait(), id)
                    console.log(`Get random BTS ...`)
                    await bocchi.sendFileFromUrl(from, `https://lolhuman.herokuapp.com/api/random/bts?apikey=${config.lol}`, id)
                    console.log(`Success Get  random bts`)
            break
            case 'exo':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                    if (!q.includes('')) return bocchi.reply(from, ind.wrongFormat(), id)
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await bocchi.reply(from, ind.wait(), id)
                    console.log(`Get random exo ...`)
                    await bocchi.sendFileFromUrl(from, `https://lolhuman.herokuapp.com/api/random/exo?apikey=${config.lol}`, id)
                    console.log(`Success Get  random exo`)
            break
            case 'blackpink':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                    if (!q.includes('')) return bocchi.reply(from, ind.wrongFormat(), id)
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await bocchi.reply(from, ind.wait(), id)
                    console.log(`Get random blackpink ...`)
                    await bocchi.sendFileFromUrl(from, `https://lolhuman.herokuapp.com/api/random/blackpink?apikey=${config.lol}`, id)
                    console.log(`Success Get random blackpink`)
            break 
            case 'meme':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                    if (!q.includes('')) return bocchi.reply(from, ind.wrongFormat(), id)
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await bocchi.reply(from, ind.wait(), id)
                    console.log(`Get random Meme ...`)
                    await bocchi.sendFileFromUrl(from, `https://lolhuman.herokuapp.com/api/meme/memeindo?apikey=${config.lol}`, id)
                    console.log(`Success Get random Meme`)
            break
            case 'darkjoke':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                    if (!q.includes('')) return bocchi.reply(from, ind.wrongFormat(), id)
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await bocchi.reply(from, ind.wait(), id)
                    console.log(`Get random Dark Jokes ...`)
                    await bocchi.sendFileFromUrl(from, `https://lolhuman.herokuapp.com/api/meme/darkjoke?apikey=${config.lol}`, id)
                    console.log(`Success Get random Dark Jokes`)
            break     
            ...
            case 'elf':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await bocchi.reply(from, ind.wait(), id)
                    console.log(`Get random Elf ...`)
                    await bocchi.sendFileFromUrl(from, `https://lolhuman.herokuapp.com/api/random/elf?apikey=${config.lol}`, id)
                    console.log(`Success Get random Elf`)
            break 
            case 'loli':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                    if (!q.includes('')) return bocchi.reply(from, ind.wrongFormat(), id)
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await bocchi.reply(from, ind.wait(), id)
                    console.log(`Get random Loli ...`)
                    await bocchi.sendFileFromUrl(from, `https://lolhuman.herokuapp.com/api/random/loli?apikey=${config.lol}`, id)
                    console.log(`Success Get random Loli`)
            break 
            case 'husbu':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                    if (!q.includes('')) return bocchi.reply(from, ind.wrongFormat(), id)
                    ...
                    await bocchi.sendFileFromUrl(from, `https://lolhuman.herokuapp.com/api/random/husbu?apikey=${config.lol}`, id)
                    console.log(`Success Get random Husbu`)
            break 
            case 'sagiri':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                    if (!q.includes('')) return bocchi.reply(from, ind.wrongFormat(), id)
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await bocchi.reply(from, ind.wait(), id)
                    console.log(`Get random Sagiri ...`)
                    await bocchi.sendFileFromUrl(from, `https://lolhuman.herokuapp.com/api/random/sagiri?apikey=${config.lol}`, id)
                    console.log(`Success Get random Sagiri`)
            break 
            case 'shinobu':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                    if (!q.includes('')) return bocchi.reply(from, ind.wrongFormat(), id)
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await bocchi.reply(from, ind.wait(), id)
                    console.log(`Get random shinobu ...`)
                    await bocchi.sendFileFromUrl(from, `https://lolhuman.herokuapp.com/api/random/shinobu?apikey=${config.lol}`, id)
                    console.log(`Success Get random shinobu`)
            break
            case 'megumin':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                    if (!q.includes('')) return bocchi.reply(from, ind.wrongFormat(), id)
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await bocchi.reply(from, ind.wait(), id)
                    console.log(`Get random megumin ...`)
                    await bocchi.sendFileFromUrl(from, `https://lolhuman.herokuapp.com/api/random/megumin?apikey=${config.lol}`, id)
                    console.log(`Success Get random megumin`)
            break
            case 'nloli':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)
                    ...
                    if (!isPremium) return await bocchi.reply(from, ind.notPremium(), id)
                    await bocchi.reply(from, ind.wait(), id)
                    console.log(`Get random nsfw loli ...`)
                    await bocchi.sendFileFromUrl(from, `https://lolhuman.herokuapp.com/api/random/nsfw/loli?apikey=${config.lol}`, id)
                    console.log(`Success Get random nsfw loli`)
            break
            case 'ecchi':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)
                    ...
                    if (!isPremium) return await bocchi.reply(from, ind.notPremium(), id)
                    await bocchi.reply(from, ind.wait(), id)
                    console.log(`Get random nsfw ecchi ...`)
                    await bocchi.sendFileFromUrl(from, `https://lolhuman.herokuapp.com/api/random/nsfw/ecchi?apikey=${config.lol}`, id)
                    console.log(`Success Get random nsfw ecchi`)
            break
            case 'baka':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                    if (!q.includes('')) return bocchi.reply(from, ind.wrongFormat(), id)
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await bocchi.reply(from, ind.wait(), id)
                    console.log(`Get random gif baka ...`)
                    await bocchi.sendFileFromUrl(from, `https://lolhuman.herokuapp.com/api/random2/baka?apikey=${config.lol}`, id)
                    console.log(`Success Get random gif baka`)
            break
            case 'neko':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
            ...
            /** case 'nhsearch':
               ...
               .then(async ({ status, result }) => {
                if (status !== 200) {
                    ...
                    await bocchi.reply(from, `*「 SEARCH HENTAI 」*\n\n➸ *Kode Nuklir*: ${result[0].id}\n\n➸ *Judul EN*: ${result[0].title_english}\n➸ *Judul JP*: ${result[0].title_japanese}\n➸ *Judul Natif*: ${result[0].title_native}\n➸ *Upload*: ${result[0].date_upload}\n➸*Page*${result[0].page}\n➸*Favorite*${result[0].favourite}`, id)
                    console.log(`Success sending ${q}!`)
                }
            })
            .catch(async (err) => {
                console.error(err)
            ...
            case 'phsearch':
                ...
                nsfw.phsearch(q)
                    .then(async ({ result }) => {
                        let phs = `*[[PORNHUB SEARCH]]*`
                        for (let i = 0; i < result.length; i++) {
                            phs +=  `\n\n➸ *Tittle*: ${result[i].title}\n➸ *Views*: ${result[i].views}\n➸ *Duration*: ${result[i].duration}\n➸ *Upload By*: ${result[i].uploader}\n➸ *Rating*: ${result[i].rating}\n➸ *Add*: ${result[i].add}\n➸ *Link*: ${result[i].link}\n\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                        }
                        phs += '\n\nBy: @hendy29_'
                        await bocchi.reply(from, phs, id)
                        console.log('Success sending Pornhub result!')
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'xnsearch':
                ...
                nsfw.xnxx(q)
                    .then(async ({ result }) => {
                        let xnxs = `*[[XNXX SEARCH]]*`
                        for (let i = 0; i < result.length; i++) {
                            xnxs +=  `\n\n➸ *Tittle*: ${result[i].title}\n➸ *Views*: ${result[i].views}\n➸ *Duration*: ${result[i].duration}\n➸ *Upload By*: ${result[i].uploader}\n➸ *Link*: ${result[i].link}\n\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                        }
                        xnxs += '\n\nBy: @hendy29_'
                        await bocchi.reply(from, xnxs, id)
                        console.log('Success sending XNXX result!')
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'nhsearch':
                ...
                nsfw.nhsearch(q)
                    .then(async ({ result }) => {
                        let nhsch = `*[[HENTAI SEARCH]]*`
                        for (let i = 0; i < result.length; i++) {
                            nhsch +=  `\n\n➸ *Kode Nuklir*: ${result[i].id}\n➸ *Judul EN*: ${result[i].title_english}\n➸ *Judul JP*: ${result[i].title_japanese}\n➸ *Judul Natif*: ${result[i].title_native}\n➸ *Upload*: ${result[i].date_upload}\n➸ *Page* : ${result[i].page}\n➸ *Favorite* : ${result[i].favourite}\n\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                        }
                        nhsch += '\n\nBy: @hendy29_'
                        await bocchi.reply(from, nhsch, id)
                        console.log('Success sending hentai result!')
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'quotesnime':
                ...
                misc.quotesnime()
                .then(async ({ result }) => {
                    await bocchi.reply(from, `➸ *Quotes*: ${result.quote}\n➸ *Character* : ${result.karakter}\n➸ *Anime* ${result.anime}`, id)
                })
            break
            // Fun
            case 'bapak': // By Kris
            ...
            misc.pantun()
            .then(async ({ result }) => {
                await bocchi.reply(from, result.pantun, id)
                console.log('Success sending pantun!')
            })
            .catch(async (err) => {
                console.error(err)
                await bocchi.reply(from, 'Error!', id)
        ...
        case 'artinama':
            ...
            .then(async ({ result }) => {
                await bocchi.reply(from, result.hasil, id)
                console.log('Success sending artinama!')
            })
            .catch(async (err) => {
                console.error(err)
                await bocchi.reply(from, 'Error!', id)
        ...
        case 'chord':
                ...
                misc.chord(q)
                    .then(async ({ result }) => {
                        ...
                        await bocchi.reply(from, result.result, id)
                        console.log('Success sending chord guitar!')
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'asupan': // hendy29_
                ...
                fun.asupan()
                    .then(async (body) => {
                       ...
                       // await bocchi.sendFileFromUrl(from, `http://sansekai.my.id/ptl_repost/${asupanx}`, 'asupan.mp4', 'Follow IG: https://www.instagram.com/hendy29_ untuk mendapatkan asupan lebih banyak.', id)
                        await bocchi.sendFileFromUrl(from, asupanx, 'asupan.mp4', 'Nih Asupan nya :v', id)
            ...
            case 'tebakgambar':
                ...
                fun.tbkgmbr()
                    .then(async ({ result }) => {
                        await bocchi.sendFileFromUrl(from, result.soalImg, 'TebakGambar.jpg', '', id)
            ...
            case 'qr': // By: hendy29_
                ...
                await bocchi.sendFileFromUrl(from, `https://api.vhtear.com/generateqr?data=${q}&apikey=${config.vhtear}`, 'qr.jpg', '', id)
                .then(() => console.log('Success creating image!'))
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Error!', id)
            ...
            case 'simi': // by: VideFrelan
                ...
                .then(async ({ status, result, message }) => {
                    if (status !== 200) {
                        ...
                        await bocchi.reply(from, result, id)
                        console.log('Success replay simi!')
                    }
                })
                .catch(async (err) => {
                    console.error(err)
            ...
            if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                console.log('Creating 8bit banner...')
                const teks18bit = q.substring(0, q.indexOf('|') - 1)
                const teks28bit = q.substring(q.lastIndexOf('|') + 2)
                await bocchi.sendFileFromUrl(from, `https://videfikri.com/api/textmaker/8bit/?text1=${teks18bit}&text2=${teks28bit}`, id)
                console.log('Success Created 8bit Banner!')
            break
            case 'wblock':
            if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q.includes('')) return bocchi.reply(from, ind.wrongFormat(), id)
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                console.log('Creating wblock banner...')
                await bocchi.sendFileFromUrl(from, `https://videfikri.com/api/textmaker/woodblock/?text=${q}`, id)
                console.log('Success Created wblock Banner!')
            break
            case 'hpotter':
            if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q.includes('')) return bocchi.reply(from, ind.wrongFormat(), id)
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                console.log('Creating hpotter banner...')
                await bocchi.sendFileFromUrl(from, `https://videfikri.com/api/textmaker/hpotter/?text=${q}`, id)
                console.log('Success Created hpotter Banner!')
            break
            case 'bnaruto':
            if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q.includes('')) return bocchi.reply(from, ind.wrongFormat(), id)
                ...
                await bocchi.sendFileFromUrl(from, `https://videfikri.com/api/textmaker/narutobanner/?text=${q}`, id)
                console.log('Success Created Naruto Banner!')
            break
            case 'kiss':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                try {
            ...
            case 'gamelogo':
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                await bocchi.sendFileFromUrl(from, `https://api.vhtear.com/gamelogo?text=${q}&apikey=${config.vhtear}`)
                console.log('Success creating image!')
            break
            break
            case 'balloonmaker':
            ...
            nsfw.bugil()
            .then(async ({ url }) => {
                await bocchi.sendFileFromUrl(from, url, 'bugil.jpg', '', id)
                console.log('Success sending foto bugil!')
            })
            .catch(async (err) => {
                console.error(err)
                await bocchi.reply(from, 'Error!', id)
        ...
        break
                ...
                .then(async ({ status, result }) => {
                    if (status === '200') {
                        await bocchi.reply(from, 'Not found.', id)
                            ...
                            await bocchi.sendFileFromUrl(from, result.thumbnail, `${result.title}.jpg`, ind.xnFound(result), id)
                            await bocchi.sendFileFromUrl(from, result.link[0].link, `${result.title}.mp4`, '', id)
                            console.log('Success sending XNXX video!')
                        ...
                        nsfw.phDl(url)
                            .then(async ({ title, download_urls, thumbnail_url }) => {
                                ...
                                if (count !== 2) {
                                    ...
                                    const shortsLow = await misc.shortener(download_urls['720P'])
                                    await bocchi.sendFileFromUrl(from, thumbnail_url, `${title}`, `Title: ${title}\n\nLinks:\n${shortsHigh} (720P)`, id)
                                        .then(() => console.log('Success sending pornhub metadata!'))
                                }
                            })
            ...
            case 'antivirtext':
                ...
                if (!isBotGroupAdmins) return await bocchi.reply(from, ind.botNotAdmin(), id)
                if (ar[0] === 'enable') {
                    if (isAntiVirtextOn) return await bocchi.reply(from, `Gagal, Anti Virtext sudah pernah dinyalakan sebelumnya`, id)
                    _antivirtext.push(groupId)
                    fs.writeFileSync('./database/group/antivirtext.json', JSON.stringify(_antivirtext))
                    await bocchi.reply(from, `*...:* *ANTI VIRTEXT*\n\nPerhatian untuk member grup ${(name || formattedTitle)}\nGroup ini telah dipasang anti virtext, jika anda mengirim virtext, maka akan otomatis terkick!`, id)
                } else if (ar[0] === '  disable') {
                    _antivirtext.splice(groupId, 1)
                    fs.writeFileSync('./database/group/antivirtext.json', JSON.stringify(_antivirtext))
                    await bocchi.reply(from, `Berhasil menonaktifkan anti-virtext`, id)
                } else {
                    await bocchi.reply(from, `Untuk melindungi grup ini dari virtext\nketik ${prefix}antivirtext on --enable\n${prefix}antivirtext off --disable`, id)
                }
            break
            case 'leveling':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
